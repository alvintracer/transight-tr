...
이 페이지는 과거 GTR adapter 문서 경로를 유지하기 위한 compatibility 문서입니다. 현재 TravelSafer core data plane은 외부 Travel Rule provider adapter가 아니라 TravelSafer Public Key Directory와 CodeVASP-compatible relay입니다.
...
1. TravelSafer가 VASP public key directory를 운영합니다.
2. 송신 기관은 수신 VASP public key로 payload를 암호화합니다.
3. TravelSafer는 암호화 payload와 상태 metadata만 relay합니다.
4. 금융기관은 TravelSafer IDC 채널을 통해 전용성 회선, VPN/IPsec, mTLS 등 기존 VAN 수준의 접속 방식을 선택합니다.
...
```json
{
  ...
  "reasonMsg": "External adapter is not enabled in the TravelSafer core data plane."
}
```
...
| Data boundary | TravelSafer가 평문 PII를 불필요하게 취급하지 않는 구조여야 합니다. |
