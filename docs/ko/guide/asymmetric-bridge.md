...
금융기관은 자체적인 규정과 보안 기준에 최적화된 통신 채널 방식을 선택적으로 활용할 수 있습니다.

| 채널 | 대상 기관 | 보안 수준 및 특징 |
|------|-----------|-------------------|
| `HTTPS` | 거래소 | TLS 1.3 + AES-256 표준 암호화 통신 |
| `mTLS` | 금융기관 | 상호 인증서 검증 및 OAuth 2.0 인가 체계 적용 |
| `VPN` | 금융기관 | IPSec 암호화 터널 기반 전용 가상 망 구성 |
| `LEASED_LINE` | 금융기관 | 물리적 망 분리 및 전용 회선 직접 연동 |

* **맞춤형 보안 연동**: 금융기관이 이미 구축해 놓은 네트워크 보안 설비에 맞추어 `mTLS`, `IPSec VPN`, `전용선 (Leased Line)` 중 적합한 채널을 통해 연동을 처리합니다.
* **전용선 연동 옵션**: 기존에 타 기관과 전용선망을 이미 구축 및 가동 중인 금융기관의 경우, 별도의 VPN 장비나 공용 인터넷 망 경유 없이 기존 전용선망을 그대로 활용하여 물리적 격리 보안을 유지할 수 있습니다.
